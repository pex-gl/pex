...
function getPackageVersion(packageName, callback) {
    exec('npm show ' + packageName + ' version', function(err, stdout, stderr) {
        var version = stdout.trim();
        console.log('   \x1b[36mmodule\x1b[0m : ' +packageName + ' @ ' + version);
...
function generatePackageInfo(callback) {
    isOnline(function(err, online) {
        ...
        async.map(pexPackages, online ? getPackageVersion : dummy, function(err, results) {
            pexPackages.forEach(function(packageName, packageIndex) {
                pkg.dependencies[packageName] = '^' + results[packageIndex];
            })
            callback(null, pkg);
        })
    })
