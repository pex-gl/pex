...
module.exports.generate = function() {
  ...
  // Pass only pex libs
  .filter(function(dir) {
    if (dir.substring(0,4) === 'pex-') return true; else return false;
  });
  // Iterate over pex libs
  pexLibs.forEach(function(dir) {
    var pathForDir = path.join(pexDir, dir);
    if (fs.lstatSync(pathForDir).isFile()) return;
    var pexLibName = dir;
    var pexLib = path.resolve(pathForDir);
    var libDirs = fs.readdirSync(pexLib);
    // Iterate over what's inside and find lib
    var libDirs = libDirs.forEach(function(dir) {
      if (dir === 'lib') {
        var pathForLib = path.join(pathForDir, dir);
        var files = fs.readdirSync(pathForLib);
        files = files.filter(function(file){
          if (path.extname(file) === '.js') return true; else return false;
        });
