...
define(["pex/core/Vec3"], function(Vec3) {
  ...
  Ray.prototype.hitTestSphere = function(pos, r) {
    ...
    hits.push(o.added(d.scaled(t0)));
    if (t0 != t1) {
      hits.push(o.added(d.scaled(t1)));
    }
  ...
  Ray.prototype.hitTestPlane = function(pos, normal) {
    ...
    var t = normal.scaled(-1).dot(this.origin.subbed(pos)) / this.direction.dot(normal);

    return [this.origin.added(this.direction.scaled(t))];
  }
