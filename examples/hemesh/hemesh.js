...
Pex.run([
  ...
  function(Core, Vec3, Window, PerspectiveCamera, Arcball, Geom, Materials, HEMesh, HEFace, HEEdge, HEVertex, CatmullClark, Extrude) {
    Window.create({
      settings: {
        width: 1280,
        height: 720,
      ...
      buildLineMesh: function(hemesh) {
        ...
        for(var i in hemesh.vertices) {
          //lineBuilder.addGizmo(hemesh.vertices[i], 0.01, Core.Color.Yellow);
        }
        ...
        for(var i in hemesh.faces) {
          ...
          for(var j=0; j<faceVertices.length; j++) {
            ...
            lineBuilder.addLine(ac, bc, edgeAB.selected ? Core.Color.Yellow : Core.Color.White);
          }          
